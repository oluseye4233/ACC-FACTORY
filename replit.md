...
- `pnpm --filter @workspace/scripts run bugmxt-latest` — run the BUGMXT SI 5-layer scan over every commit since the last scan. Report lands in `docs/bugmxt/BUGMXT_Report_<date>_<base>-<head>.{md,pdf}`; scan pointer in `docs/bugmxt/.last-scan.json` advances only on success; verified false positives carry forward via `docs/bugmxt/TRIAGE_NOTES.md` (see `docs/bugmxt/README.md`).
- `pnpm --filter @workspace/scripts run sync-github` — mirror new local commits to GitHub (`oluseye4233/ACC-FACTORY@main`). Runs automatically every 10 minutes via the always-on `GitHub Sync` workflow; it is intentionally excluded from `scripts/post-merge.sh` because network sync can exceed the merge setup timeout. Incremental one-commit-per-sync via the GitHub Data API (never force-pushes; resumes from the `Replit-Commit:` trailer stamped on the remote head). `.github/workflows/*` is intentionally excluded — the connector OAuth token lacks the `workflow` scope and GitHub rejects pushes touching those files; update Actions files on GitHub directly if needed. If the trailer is ever missing (fresh repo / manual remote commits rewrote it away), run once with `-- --base=<local sha matching the remote tree>`; if local history was rewritten and no matching base exists, run the one-time full reconciliation with `-- --snapshot`.
...
**Scheduled jobs (Replit Scheduled Deployments).** Five cron endpoints (`POST /api/cron/send-weekly-digest`, `POST /api/cron/reset-harness-limits`, `POST /api/cron/run-f0-monitoring`, `POST /api/cron/sweep-cost-cap-alerts`, `POST /api/cron/reconcile-f10-deployments`) are driven externally via `pnpm --filter @workspace/scripts run cron-tick -- <target>` (`weekly-digest` Mondays 09:00 UTC; `reset-harness-limits` daily 00:00 UTC; `run-f0-monitoring` weekly, e.g. Mondays 08:00 UTC; `sweep-cost-cap-alerts` every 15 minutes, `*/15 * * * *`; `reconcile-f10-deployments` every 5 minutes, `*/5 * * * *`). The script needs `PUBLIC_BASE_URL` + `CRON_SECRET` and exits non-zero on any failure so a missed tick surfaces in deployment logs. **The autoscale deployment must be published with Public visibility** — a Private deployment's Replit auth wall 307-redirects external requests, making `/api/cron/*` unreachable; `cron-tick` detects the wall redirect and fails with an explicit error. Scheduled Deployments are created by the user in the Publishing tool (one per target, UTC cron expressions: digest `0 9 * * 1`, reset `0 0 * * *`, f0 `0 8 * * 1`, cost-cap sweep `*/15 * * * *`, F10 reconciliation `*/5 * * * *`) — see `docs/ATANDA_Command_Centre_MVP_Publish_Ready_v4.md` §5. Without `reset-harness-limits` every tier holder locks at their daily cap after 24h. `run-f0-monitoring` sweeps every ACTIVE F0 retainer, persists a run to `f0_monitoring_runs`, and emails the owner on a breach (any eventAlert urgency ACT_SOON/ACT_NOW); it skips retainers already swept within 6 days and halts LLM spend once the company-wide monthly cost cap is reached. Breaches surface as OPEN alerts (with last-run time) on the F0 dashboard until acknowledged; alert email is gated on the owner's personal-scope `retainerAlertsEnabled` notification preference. **Cron dead-man's-switch:** every successful `/api/cron/*` handler records a heartbeat in `cron_tick_status`; an in-process monitor (`src/lib/cron-heartbeat.ts`, 15-min interval, no-op under vitest) emails `ADMIN_EMAILS` exactly once per stale episode (UNIQUE `(target, stale_since_key)` stamp in `cron_stale_notifications`) when a target misses its window (F10 reconciliation 20m; cost-cap sweep 60m; reset 48h; digest/f0 14d). When a stale target ticks again, a one-time "all clear" recovery email closes the loop (stamp atomically claimed via `recovered_notified_at`; no recovery email if the stale alert itself was never sent, and stamps are not claimed while `ADMIN_EMAILS` is empty). Admins see per-target OK/STALE/NEVER-TICKED status at `/admin/ops` (`GET /api/admin/cron-status`), plus a rolling tick history per target (bounded `cron_tick_events` table, pruned to the newest 200 rows per target on every tick) surfacing an "expected N, got M in last 24h" flakiness line and a recent-tick strip — so intermittently-failing schedules are visible, not just dead ones.
...
- **F10 provider authorization keys:** `F10_PROVIDER_AUTHORIZATION_KEY_VERSION` + `F10_PROVIDER_AUTHORIZATION_KEY` are required together; the key is a base64url-encoded secret of at least 32 bytes. During rotation, move the old pair to `F10_PROVIDER_AUTHORIZATION_PREVIOUS_KEY_VERSION` + `F10_PROVIDER_AUTHORIZATION_PREVIOUS_KEY`, configure the new current pair, deploy, and keep the previous pair until every active connection has been opened and lazily re-encrypted. Keep `SESSION_SECRET` available while legacy `v1` connection references are being migrated. Missing historical keys pause reconciliation and require reconnecting the affected provider account; signed acceptance receipts are not changed.
