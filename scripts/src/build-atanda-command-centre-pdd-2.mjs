// Generates the ATANDA Command Centre PDD 2 (Investor Edition) as a PDF.
// Run with:  node scripts/src/build-atanda-command-centre-pdd-2.mjs
// Output:    docs/ATANDA_Command_Centre_PDD_2.pdf
//
// PDD 2 supersedes the original investor PDD after the "Ontological Debug 1"
// pass. All terminology, acronym expansions, and certification cutoffs are
// aligned to:
//   - GENERAL_TECHNICAL_TERMS_REGISTRY v1.0
//   - MASTER_SPC_PLATFORM_REGISTRY v1.0
// and to the corrected server-side mappings now shipping in
//   artifacts/api-server/src/engines/{prompts,shared,f1,de}.ts.
//
// This is an F6-shaped 4-Part ATLAS Project Definition Document
// (cheatSheet / execSummary / worksheet / implementation).
...
const OUT_PATH = resolve(repoRoot, "docs/ATANDA_Command_Centre_PDD_2.pdf");
...
const doc = new PDFDocument({
  ...
  info: {
    Title: "ATANDA Command Centre PDD 2 (Investor Edition)",
    Author: "ATANDA Command Centre",
    Subject: "Second-issue Project Definition Document — registry-aligned, FROM_CODEBASE, produced via the FORGE.BONSAI HARNESS (F6 ATLAS Drafter)."
  }
});
...
function calloutBox(title, body, accent = GOLD) {
  ...
  const w = doc.page.width - doc.page.margins.left - doc.page.margins.right;
  const startY = doc.y;
  doc.save().fillColor(accent).rect(x, startY, 4, 0).fill().restore(); // placeholder
...
doc.moveDown(0.2);
doc.fillColor(GOLD).font("Helvetica-Bold").fontSize(13).text("PDD 2  ·  4-PART ATLAS  ·  INVESTOR EDITION", { characterSpacing: 2 });
doc.moveDown(0.4);
doc.fillColor(SOFT).font("Helvetica-Oblique").fontSize(12).text("Produced via the FORGE.BONSAI HARNESS (F6 — ATLAS Drafter). Registry-aligned re-issue.");
doc.moveDown(2);
hr(ACCENT);

h3("Document classification");
kv([
  ["Issue", "PDD 2 — supersedes the original investor PDD"],
  ["Audience", "Investors, board members, strategic partners"],
  ["Origin", "FORGE.BONSAI HARNESS · F6 ATLAS PDD format (4-Part)"],
  ["Source artifact", "This monorepo (artifacts/api-server, artifacts/command-centre, lib/*)"],
  ["Mode", "FROM_CODEBASE — grounded entirely in shipped code"],
  ["Nomenclature anchor", "GENERAL_TECHNICAL_TERMS_REGISTRY v1.0 + MASTER_SPC_PLATFORM_REGISTRY v1.0"],
  ["Generated", new Date().toISOString().slice(0, 10)]
]);

calloutBox(
  "What changed in PDD 2",
  "This re-issue corrects six terminology and scoring drifts identified during the Ontological Debug 1 pass. (1) JCSE is the Junglenomics Composite Score Estimate — the composite AI agent quality score (0–50). (2) HIVE certification bands are PLATINUM ≥48 / GOLD ≥43 / SILVER ≥36 / BRONZE ≥30 (the prior PDD quoted 45/40/35/30; the server-side deterministic mapping in shared.ts#certTierForJcse now matches the registry). (3) The 7 pillars belong to the CONTEXT CRAFT framework; ATLAS is reserved for the 4-Part PDD format itself. (4) MA = Micro Agent (single-function, 1–3 prompt ceiling), not 'Molecular Agent'. (5) GRO = Golden Rule Orchestrator (Five-State Ethical Governance Orchestrator). (6) SPC = Super Prompt Card; CELL SI = Lightweight Logic Agent Cultivation and Factory."
);
...
p("Every claim in this PDD is traceable to a real file in this codebase. If you are an investor, read Part 2 first; the technical depth in Parts 3 and 4 is there as evidence that the team has actually built what Part 2 claims. The HARNESS that produced this document is itself the product being described — this is a real deliverable from our own software, not marketing collateral. Where this PDD uses HARNESS-specific terminology (JCSE, HIVE, CONTEXT CRAFT, GRO, SPC, MA, ATLAS, SPARTAN, CELL SI), the meanings are fixed by the registry citations on this page.");
...
p("A subscription SaaS portal that productises the FORGE.BONSAI HARNESS — a seven-step instruction layer (F1 → F7, plus two power-user engines F6-VDJ and DE-SPC) that walks a creator from a messy prompt all the way to a SPARTAN-certified MVP-PDD with a public verification URL. Every step is run by Claude Sonnet 4 against a deterministic, version-pinned system prompt; every artifact is persisted and counted; every certified output gets its own public verify URL. Quality is measured by the Junglenomics Composite Score Estimate (JCSE) — a 0–50 composite scored across the seven CONTEXT CRAFT pillars — and certified to a HIVE band: BRONZE (≥30), SILVER (≥36), GOLD (≥43), PLATINUM (≥48).");
...
bullets([
  ...
  "≥ 60% of certified MVP-PDDs reach a HIVE band of SILVER or higher (JCSE ≥ 36), measured against the registry-anchored cutoffs."
]);
...
bullets([
  ...
  "ARCHITECT — $199/mo or $1,910/yr. 100 sessions/day, F1–F7 with public verify URLs, DE-SPC engine (gated by the ASPE badge), exportable Micro Agent (MA) Birth Packages, dedicated account manager.",
...
bullets([
  ...
  "Exemplar library of 9 reference Super Prompt Cards (SPCs) with unique DISC fingerprints — every one a fork-able starting point for a new session.",
  "Public verify route at /api/verify and a verify page in the portal. MVP-PDDs are externally citable from day one.",
  "Registry-aligned scoring: the deterministic server-side mapping certTierForJcse in artifacts/api-server/src/engines/shared.ts emits HIVE bands at 30 / 36 / 43 / 48, matching the registry and the F1/F2 prompt calibration text."
]);
...
bullets([
  ...
  "Risk: a foundation-model vendor ships their own methodology layer. Mitigation: ours is grounded in seven years of original methodology IP (the seven CONTEXT CRAFT pillars, the HIVE 14-D certification frame, the GRO five-state ethical contract, the CELL SI doctrine) and the public-verify-URL distribution flywheel creates a network effect they would have to rebuild from zero.",
  ...
  "Risk: terminology drift between docs, codebase, and registry. Mitigation: the Ontological Debug 1 pass (which produced this PDD 2) aligned all server prompts, deterministic mappings, UI strings, and PDF builders to the GENERAL_TECHNICAL_TERMS_REGISTRY v1.0 and MASTER_SPC_PLATFORM_REGISTRY v1.0 in a single coordinated commit set."
]);
...
phaseBlock({
  ...
  status: "COMPLETE — in production",
  deliverables: [
    "F1 (Prompt Diagnosis) — workspace at /command with a 7-pillar CONTEXT CRAFT rubric and JCSE grading (0–50) producing a HIVE band (BRONZE ≥30, SILVER ≥36, GOLD ≥43, PLATINUM ≥48).",
    "F2 (Atomic Prompt) — converts diagnosis to a structured Atomic Prompt artifact, re-scored against the same CONTEXT CRAFT rubric.",
    "F3 (Micro Agent (MA) Birth Package) — packages prompt + tools + handoffs under the CELL SI (Lightweight Logic Agent Cultivation and Factory) doctrine; can escalate to F5 directly.",
    "F4 (Micro PDD) — short product spec generated from the MA Birth Package.",
    "F5 (Build SPC) — full 15-section Super Prompt Card (SPC), including the GRO (Golden Rule Orchestrator) five-state ethical contract, with optional FROM_F3 escalation path.",
    ...
    "DE-SPC (Digitally Evolved SPC, /api/harness/evolve) — gated by requireAspeBadge; evolves an existing SPC into an adaptive multi-agent SPC by fusing prior Micro Agent (MA) Birth Packages from the same session."
  ],
  exit: "All nine engines callable end-to-end; per-engine telemetry recorded to harness_engine_runs on every Claude call; tier gates correctly block free-tier users from F5+ unless they have an active escalation row; deterministic HIVE band mapping (shared.ts#certTierForJcse) matches the F1/F2 prompt calibration text."
});
...
phaseBlock({
  ...
  status: "COMPLETE — in production",
  deliverables: [
    "Exemplar library (/exemplars) with 9 seeded reference Super Prompt Cards (SPCs), each with a DISC fingerprint, JCSE tier chip, and Fork-to-session CTA.",
    "Public /verify route resolving MVP-PDDs by certificate slug; returns 200 with the rendered MVP-PDD or 404.",
    "Quest badges surface (/quests) with live progress for ASPE, AISA, AISE; AISE claim form uses a private-IP-blocked, SSRF-hardened URL verifier.",
...
phaseBlock({
  ...
  deliverables: [
    ...
    "Ontological Debug 1 pass: all engine prompts, the deterministic certTierForJcse mapping, UI labels, FAQ copy, and PDF builders aligned to the GENERAL_TECHNICAL_TERMS_REGISTRY v1.0 and MASTER_SPC_PLATFORM_REGISTRY v1.0."
  ],
...
h2("Registry-anchored vocabulary");
p("PDD 2 fixes the prior PDD's six-term drift. The following table is now the citation surface for every other doc, prompt, and UI string in the codebase.");
kv([
  ["JCSE", "Junglenomics Composite Score Estimate — composite AI agent quality score, 0–50, computed from the 7 CONTEXT CRAFT pillars"],
  ["HIVE bands", "PLATINUM ≥48 · GOLD ≥43 · SILVER ≥36 · BRONZE ≥30 (deterministic, server-side, certTierForJcse in shared.ts)"],
  ["CONTEXT CRAFT", "Seven-Pillar Context Engineering Framework — the rubric F1 grades against"],
  ["ATLAS", "4-Part PDD format (CHEAT SHEET / EXEC SUMMARY / WORKSHEET / IMPLEMENTATION). Drafted by F6."],
  ["SPC", "Super Prompt Card — the 15-section artifact F5 produces"],
  ["MA", "Micro Agent — single-function agent with a 1–3 prompt ceiling. F3 emits an MA Birth Package."],
  ["CELL SI", "Lightweight Logic Agent Cultivation and Factory — the 8-organelle doctrine that powers F3"],
  ["GRO", "Golden Rule Orchestrator — Five-State Ethical Governance Orchestrator embedded in every F5 SPC"],
  ["SPARTAN", "The audit-grade compression math F7 applies to convert an ATLAS PDD into an MVP-PDD"],
  ["MVP-PDD", "The final certified deliverable; surfaced at a public verify URL"]
]);
...
export const harnessArtifactsTable = pgTable("harness_artifacts", {
  ...
  certTier:     text({ enum: ["NONE","BRONZE","SILVER","GOLD","PLATINUM"] }),
...
calloutBox(
  "Note on existing certTier values",
  "Historical harness_artifacts rows keep whatever certTier they were stamped with at the time. Only new computations use the registry-aligned HIVE cutoffs (≥48 / ≥43 / ≥36 / ≥30). A pre-existing artifact that scored 46 is preserved as GOLD; a new artifact scoring 46 will also be GOLD under the new cutoffs. No backfill was performed by design — the per-artifact certTier is a historical record of what the system stamped at issue time, not a retroactive judgement."
);
...
p("Both code paths copy artifact payloads, but only ever from the authenticated user's own corpus (or the public exemplar library, which is intentionally public). No cross-user read path exists. DE-SPC specifically synthesises Micro Agent (MA) Birth Packages from a single session — the SQL filter is dual-bound on (userId, sessionId).");
...
h3("Ethical governance — the GRO contract");
p("Every F5 Super Prompt Card embeds a GRO (Golden Rule Orchestrator) five-state ethical governance section. This is not a runtime middleware — it is a contractual section of the SPC artifact that downstream implementers commit to honouring. The HARNESS does not enforce it at request time; it enforces it at issuance time by refusing to mint an SPC whose GRO section is empty or trivially passing.");
...
bullets([
  ...
  "Per-engine telemetry surfaceable via the harness_engine_runs table; this is the analytic warehouse from which we tune CONTEXT CRAFT rubrics."
]);

h2("Why the architecture deserves the round");
p("Three things, all reinforced by the codebase: (1) the contract-first OpenAPI discipline means we can ship the same product against multiple front-ends without a rewrite — first React, next year mobile and CLI. (2) The vendor-neutral LLM proxy means a Claude price hike or model deprecation is a one-line change. (3) The per-engine telemetry is a flywheel: every paying user makes the next user's output measurably better, and the data is ours. The Ontological Debug 1 pass that produced this PDD 2 demonstrates the fourth, less visible asset: nomenclature discipline. The codebase, the prompts, the deterministic scoring, the UI copy, the FAQ, and the investor PDD now cite the same two registries — a quality bar most AI-tooling vendors at this stage do not meet.");

doc.moveDown(1);
hr(ACCENT);
doc.fillColor(SOFT).font("Helvetica-Oblique").fontSize(9).text(
  `Generated ${new Date().toISOString().slice(0, 10)} · ATANDA Command Centre · PDD 2 · 4-Part ATLAS · Investor Edition · Produced via the FORGE.BONSAI HARNESS (F6) · Registry: GENERAL_TECHNICAL_TERMS_REGISTRY v1.0 + MASTER_SPC_PLATFORM_REGISTRY v1.0`,
  { align: "center" }
);
