...
const DOCTRINE_NOTE = [
  "> **Doctrine.** CODE ORACLE (the F8 engine of the FORGE.BONSAI HARNESS) is the",
...
/** The universal AGENTS.md — the CODE ORACLE operating brief for any IDE agent. */
export function buildAgentsMd(
  ...
  return [
    "# CODE ORACLE — Agent Operating Brief",
    "",
    DOCTRINE_NOTE,
    "",
    "## What this project is",
    "",
    `A deploy-ready scaffold generated by **CODE ORACLE (F8)** for **${platformLabel(bundle.platform)}**, ` +
    ...
    "",
    bundle.notes ? `## CODE ORACLE notes\n\n${bundle.notes}\n` : "",
  ].join("\n");
}
...
function cursorMdc(): string {
  return [
    "---",
    "description: CODE ORACLE operating brief — build in fidelity to the certified MVP-PDD",
    "globs:",
    "alwaysApply: true",
    "---",
    "",
    "Follow the CODE ORACLE operating brief in `AGENTS.md` at the project root.",
...
function shortBrief(bundle: CodebaseBundle): string {
  return [
    ...
    bundle.manifest.buildCommand
      ...
      : "- Build: none",
    "",
    "Read `AGENTS.md` for the full CODE ORACLE operating brief: the certified spec,",
...
function claudeMd(bundle: CodebaseBundle): string {
  return [
    "# CLAUDE.md",
    "",
    "This project was scaffolded by CODE ORACLE (the F8 engine of the FORGE.BONSAI",
...
function replitMd(bundle: CodebaseBundle): string {
  return [
    ...
    "",
    "Scaffolded by CODE ORACLE (F8) from a SPARTAN-certified MVP-PDD. Continue",
...
function copilotMd(bundle: CodebaseBundle): string {
  return [
    ...
    "",
    "Build in fidelity to the CODE ORACLE operating brief in `AGENTS.md`. This",
    "codebase is a CODE ORACLE (F8) scaffold of a SPARTAN-certified MVP-PDD.",
    "",
    shortBrief(bundle),
  ].join("\n");
}
...
function windsurfRules(bundle: CodebaseBundle): string {
  return [
    "Follow the CODE ORACLE operating brief in AGENTS.md.",
    "This is an F8 CODE ORACLE scaffold of a SPARTAN-certified MVP-PDD; the HARNESS",
