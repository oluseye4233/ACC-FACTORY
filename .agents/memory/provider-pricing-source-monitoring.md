...
Monitor official provider rate-card text as a review signal; a page fingerprint change is not proof that the model's price changed. Keep source acceptance separate from production pricing edits so those edits remain visible in normal code review. Do not accept a new fingerprint while a configured model mention is missing; first confirm the provider's model status and update the reviewed model list.
